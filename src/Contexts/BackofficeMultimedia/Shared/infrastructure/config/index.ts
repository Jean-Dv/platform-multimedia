...
const backofficeMultimediaConfig = convict({
  env: {
  ...
  mongo: {
    url: {
      ...
      format: String,
      env: 'MONGO_URL',
      default: 'mongodb://localhost:27017/backoffice-multimedia-backend-dev'
    }
  },
  rabbitmq: {
    connectionSettings: {
...
const getConfig: string = backofficeMultimediaConfig.get('env')

backofficeMultimediaConfig.loadFile([
  path.join(__dirname, `./default.json`),
  path.join(__dirname, `./${getConfig}.json`)
])

export default backofficeMultimediaConfig
