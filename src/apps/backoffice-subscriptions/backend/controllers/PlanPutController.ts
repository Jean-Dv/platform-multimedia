...
import { CreateBackofficePlanCommand } from '@BackofficeSubscriptions/Plans/application/Create/CreateBackofficePlanCommand'
...
export class PlanPutController implements Controller {
  ...
  public async run(req: PlanPutRequest, res: Response): Promise<void> {
    try {
      ...
      const createPlanCommand = new CreateBackofficePlanCommand({
        id,
        name,
        price,
        duration,
