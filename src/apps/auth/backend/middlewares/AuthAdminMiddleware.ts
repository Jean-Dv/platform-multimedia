...
export class AuthAdminMiddleware implements Middleware {
  ...
  public async run(
    ...
    try {
      ...
      if (!(await this.validateRoleAdmin(userId, roleId))) {
        res.status(httpStatus.UNAUTHORIZED).json({
          ok: false,
          data: {
            message: 'Invalid token'
  ...
  private async validateRoleAdmin(
    userId: string,
    roleId: string
  ): Promise<boolean> {
    ...
    return roleName === role.name && role.name === 'admin'
  }
}
